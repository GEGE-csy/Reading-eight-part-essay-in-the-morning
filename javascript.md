...
数据绑定：目标对象的属性发生变化时触发相应操作

验证和过滤输入：拦截对象的赋值操作，在赋值之前校验是否合法

缓存和延迟加载：访问某个属性时在get中检查缓存中是否有值
...
## 21. es6模块和commonjs模块有什么异同

异：

- es6模块使用import和export，commonjs模块使用require和module.exports

- es6模块导出的是值的引用，commonjs模块导出的是值的拷贝

- es6导入的模块是只读的，commonjs导入的模块是可以重新赋值的
...
## 9. setTimeout、setInterval、requestAnimationFrame各有什么特点
